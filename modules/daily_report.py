...
def _reports_dir():
    return Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / 'data' / 'reports'
...
def _find_daily_report_path(report_date_str):
    """Return the path to an existing daily report for YYYY-MM-DD.

    Prefers the all-stores report (no _store suffix); falls back to any match.
    """
    reports_dir = _reports_dir()
    if not reports_dir.exists():
        return None
    exact = reports_dir / f'Daily_Alerts_Report_{report_date_str}.xlsx'
    if exact.exists():
        return str(exact)
    matches = sorted(reports_dir.glob(f'Daily_Alerts_Report_{report_date_str}*.xlsx'))
    return str(matches[0]) if matches else None


def _daily_report_marker_path(report_date_str):
    """Sidecar marker file recording that the email for a date was sent."""
    return _reports_dir() / f'Daily_Alerts_Report_{report_date_str}.emailed'


def _daily_report_emailed_for_date(report_date_str):
    """Return True if the daily report email for YYYY-MM-DD was already sent."""
    return _daily_report_marker_path(report_date_str).exists()


def _mark_daily_report_emailed(report_date_str):
    """Record that the daily report email for YYYY-MM-DD was sent successfully."""
    try:
        path = _daily_report_marker_path(report_date_str)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(datetime.now(IST).isoformat())
    except Exception as e:
        logger.warning(f"Could not write emailed marker for {report_date_str}: {e}")


def _is_email_configured(email_config=None):
    """Return True if SMTP host and at least one recipient are configured."""
    cfg = email_config if email_config is not None else get_email_config()
    return bool(cfg.get('smtp_host') and cfg.get('recipients'))


def _send_with_retry(filepath, email_config, report_subject=None, report_body=None,
                     attempts=3, delay_seconds=60):
    """Send the report email, retrying transient failures a few times."""
    for attempt in range(1, attempts + 1):
        if send_report_email(filepath, email_config, report_subject, report_body):
            return True
        if attempt < attempts:
            logger.warning(
                f"Email send attempt {attempt}/{attempts} failed; retrying in {delay_seconds}s"
            )
            time.sleep(delay_seconds)
    return False


def _email_existing_daily_report(report_date_str):
    """Email an already-generated daily report for YYYY-MM-DD.

    Returns one of: 'sent', 'failed', 'not_configured', 'missing'.
    Writes the emailed marker on success.
    """
    filepath = _find_daily_report_path(report_date_str)
    if not filepath:
        logger.warning(f"No daily report file found for {report_date_str}; cannot email")
        return 'missing'

    email_config = get_email_config()
    if not _is_email_configured(email_config):
        logger.warning("Email not configured - existing daily report saved locally only")
        return 'not_configured'

    subject = f'Daily AI Alerts Report - {report_date_str}'
    body = f"""Dear Team,

Please find attached the Daily AI Alerts Report for {report_date_str}.
...
Sakshi.AI System"""
    ...
    logger.info(f"Sending existing daily report for {report_date_str} to {email_config.get('recipients')}")
    if _send_with_retry(filepath, email_config, subject, body):
        _mark_daily_report_emailed(report_date_str)
        logger.info(f"Existing daily report for {report_date_str} emailed successfully")
        return 'sent'
    return 'failed'
    ...
    try:
        ...
        if filepath:
            ...
            if not email_config.get('smtp_host'):
                ...
                success = _send_with_retry(filepath, email_config)
                if success:
                    _mark_daily_report_emailed(report_date)
                    logger.info("Daily report email sent successfully")
                else:
                    logger.warning("Failed to send daily report email after retries (see logs for details)")
            return True
        else:
    ...
    def scheduler_loop():
        ...
        while True:
            try:
                ...
                # for yesterday. The report file existing is NOT enough: the email
                # may never have been sent (e.g. the instance died right after
                # writing the file, or the SMTP send failed). Email status is
                # tracked separately via a sidecar marker.
                if now >= target_today:
                    report_date = (now - timedelta(days=1)).strftime('%Y-%m-%d')
                    file_exists = _daily_report_exists_for_date(report_date)
                    already_emailed = _daily_report_emailed_for_date(report_date)

                    if file_exists and already_emailed:
                        logger.info(
                            f"Daily report for {report_date} already generated and emailed; "
                            f"scheduling next run for tomorrow"
                        )
                        target = target_today + timedelta(days=1)
                    elif file_exists and not already_emailed:
                        logger.warning(
                            f"Daily report for {report_date} exists but was not emailed; sending now"
                        )
                        status = _email_existing_daily_report(report_date)
                        if status in ('sent', 'not_configured'):
                            target = target_today + timedelta(days=1)
                        else:
                            target = now + timedelta(minutes=30)
                            logger.warning(
                                "Catch-up email for existing daily report failed; retrying in 30 minutes"
                            )
                    else:
                        logger.warning(
                        ...
                        done = _daily_report_exists_for_date(report_date) and (
                            _daily_report_emailed_for_date(report_date) or not _is_email_configured()
                        )
                        if done:
                            target = target_today + timedelta(days=1)
                        else:
                            # Retry in 30 minutes if generation or email failed.
                            target = now + timedelta(minutes=30)
                            logger.warning(
                                "Catch-up daily report run did not complete (generate + email); "
