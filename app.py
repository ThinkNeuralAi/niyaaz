...
from modules.daily_report import start_daily_report_scheduler, start_monthly_report_scheduler
...
@login_required
def serve_material_theft_snapshot(filename):
    """Serve material theft snapshot/GIF files"""
    return send_from_directory('static/material_theft_snapshots', filename)
...
def api_generate_monthly_report():
    """Manually trigger monthly alert report generation and email (enabled by default).

    Expected JSON body:
    {
      "month": "2026-04",  # optional, defaults to previous month
      "store_id": "...",   # optional
      "send_email": true     # optional, defaults true
    }
    """
    try:
        ...
        target_month = request.json.get('month') if request.is_json else None
        store_id = request.json.get('store_id') if request.is_json else None
        filepath = generate_monthly_report(app, db_manager, target_month=target_month, store_id=store_id)
        if not filepath:
            return jsonify({'success': False, 'error': 'No active stores or report generation failed'})
...
Regards,
Sakshi.AI System"""
            sent = send_report_email(filepath, email_config, report_subject=subject, report_body=body)
            result['email_sent'] = sent
        else:
            result['email_sent'] = False
            result['email_skipped'] = True
...
@app.route('/api/download_monthly_report')
def api_download_monthly_report():
    """Download the latest or a specific month's monthly report"""
    from pathlib import Path
    ...
    if report_month:
        store_suffix = f'_{store_id}' if store_id else ''
        filename = f'Monthly_Alerts_Report_{report_month}{store_suffix}.xlsx'
    else:
        reports_path = Path(reports_dir)
        if not reports_path.exists():
            return jsonify({'error': 'No reports found'}), 404
        files = sorted(reports_path.glob('Monthly_Alerts_Report_*.xlsx'), reverse=True)
        if not files:
            return jsonify({'error': 'No reports found'}), 404
        filename = files[0].name
...
if __name__ == '__main__':
    ...
    # Start monthly report scheduler (sends previous month Excel at 10:00 AM IST on day 1)
